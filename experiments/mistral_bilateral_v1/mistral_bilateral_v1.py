#!/usr/bin/env python3
"""
mistral_bilateral_v1.py — EXP_MISTRAL_BILATERAL_V1
4th-architecture replication of bilateral oracle L1 + L2 false certainty
on Mistral-7B-Instruct-v0.3.

SCIENTIFIC QUESTION:
  Does bilateral oracle Fisher+PCA64 replicate on Mistral-7B-Instruct-v0.3?
...
DESIGN:
  Phase 1 — Bilateral Oracle L1 (N=200/class, pool=15000, shuffled control):
    ...
    Fisher+PCA64 at penultimate layer (L30 of 32) step-1.
  ...
  Phase 2 — L2 False Certainty (N=100/class, entropy-matched):
    ...
    ENTROPY FIX: cast logits to float32 + nan_to_num before softmax;
    filter NaN/non-finite values from calibration sample.
...
TERMINATION RULES:
  ...
  T2: L2 gap < 0.05 → L2 does not generalize to Mistral, note scope contraction

GPU: T4 (~14 hrs), 4-bit NF4 required (Mistral-7B-7B does not fit T4 in fp16)
Architecture: 32 transformer layers, 4096 hidden dim, 32 attn heads (8 KV = GQA)
Paper A context: 4th architecture alongside Qwen, Llama, Gemma.
"""

from __future__ import annotations
import subprocess, sys
subprocess.run([sys.executable, "-m", "pip", "install", "-q", "bitsandbytes>=0.46.1"], check=True)
...
LAYER_IDX        = None          # auto-selected as n_layers - 2 (penultimate = 30)
...
CTX_MIN_F1_WC    = 0.50
CW_MAX_F1        = 0.05
ENT_HALF         = 0.30          # half-width of entropy window; Mistral distribution tighter

SAVE_PATH        = "/kaggle/working/mistral_bilateral_v1_results.json"
INTERMEDIATE     = "/kaggle/working/mistral_bilateral_v1_intermediate.json"
...
# ── Layer resolver (Mistral uses model.model.layers → path "model.layers") ───────
_LAYER_PATHS = [
    "model.layers",
...
def get_step1_entropy(model, tokenizer, prompt: str):
    """
    Compute token-distribution entropy at the last prompt position.

    ENTROPY FIX: cast logits to float32 and apply nan_to_num before softmax.
    Mistral (and Gemma-2) can produce extreme logit values in float16 that
    become NaN after softmax → entropy = NaN → calibration window collapses.
    The float32 cast + nan_to_num guards against this without altering the
    distribution for well-behaved logits.
    """
    ids = tokenizer(prompt, return_tensors="pt").input_ids.to(DEVICE)
    with torch.no_grad():
        out = model(ids)
...
def run_phase2(model, tokenizer, pool, layer_idx, bo_pca, bo_lda):
    ...
    # Calibrate theta_conf: 30th-percentile entropy on pool sample.
    # NaN filtering is critical — float16 logits can produce NaN entropy even
    # with the float32 fix on edge-case tokens. Filter before percentile.
    ...
    if not sample_ents:
        raise RuntimeError("All entropy values non-finite during calibration. Check model logits.")
    finite_frac = len(sample_ents) / 500
    theta_conf  = float(np.percentile(sample_ents, 30))
    ent_lo      = theta_conf - ENT_HALF
    ent_hi      = theta_conf + ENT_HALF
    print(f"  sample_ents: {len(sample_ents)}/500 finite ({finite_frac:.1%})", flush=True)
    ...
    for item in pool:
        ...
        pnc = prompt_nocontext(tokenizer, q)
        ent = get_step1_entropy(model, tokenizer, pnc)
        if not np.isfinite(ent) or not (ent_lo <= ent <= ent_hi):
            continue
...
def main():
    ...
    # Set layer index to penultimate (32 layers → L30)
    global LAYER_IDX
    ...
    # Summary
    results = {
        "experiment":    "EXP_MISTRAL_BILATERAL_V1",
        "model":         MODEL_ID,
        "layer_idx":     LAYER_IDX,
        "n_layers":      n_layers,
        "pca_dim":       PCA_DIM,
        ...
        "architecture_note": (
            "Mistral-7B-Instruct-v0.3 (GQA, SwiGLU, RoPE, sliding-window attn, "
            "Mistral family); 4-bit NF4 quantization; 32 layers; 4096 hidden dim"
        ),
    }
    ...
    if l1_auroc < 0.65:
        ...
        results["termination"] = "T2_L2: L2 gap < 0.05; L2 does not generalize to Mistral"
    else:
        results["termination"] = "NO_TERMINATION"

    print(f"\n{'='*50}", flush=True)
    print(f"EXP_MISTRAL_BILATERAL_V1 RESULTS", flush=True)
