#!/usr/bin/env python3
"""
ood_generalization_v1.py — EXP_OOD_GENERALIZATION_V1
...
def collect_trivia_bilateral(model, tokenizer, pool, n_target=N_SOURCE):
    ...
    for item in pool:
        ...
        if f1_nc >= PARAM_MIN_F1 and len(param_items) < n_target:
            hs, ent = get_hs_and_entropy(model, tokenizer, p_nc)
            if hs is not None:
                param_items.append({"hs": hs, "ent": ent})
        elif f1_nc <= CTX_DEP_MAX_F1 and len(ctx_dep_items) < n_target:
            # With-context pass (use Wikipedia answer context approximation)
            # We use the gold answer as a cue in context to verify context-dependence
            context = f"The answer to this question is: {golds[0]}."
            p_wc = prompt_wc(tokenizer, q, context)
            ans_wc = generate(model, tokenizer, p_wc)
            f1_wc = token_f1(ans_wc, golds)
            if f1_wc >= PARAM_MIN_F1:
                hs, ent = get_hs_and_entropy(model, tokenizer, p_nc)
                if hs is not None:
...
def collect_mmlu_items(model, tokenizer, n_target=N_OOD_TRANSFER):
    ...
    for subj in MMLU_STEM_SUBJECTS:
        ...
        for row in items:
            ...
            if f1_nc >= PARAM_MIN_F1 and len(knows_items) < n_target:
                hs, ent = get_hs_and_entropy(model, tokenizer, p_nc)
                if hs is not None:
                    knows_items.append({"hs": hs, "ent": ent})
            elif f1_nc <= CTX_DEP_MAX_F1 and len(doesnt_items) < n_target:
                hs, ent = get_hs_and_entropy(model, tokenizer, p_nc)
                if hs is not None:
...
def collect_hotpotqa_bilateral(model, tokenizer, n_target=N_OOD_TRANSFER):
    ...
    for row in ds:
        ...
        if f1_nc >= PARAM_MIN_F1 and len(param_items) < n_target:
            ...
            if hs is not None:
                param_items.append({"hs": hs, "ent": ent})
        elif f1_nc <= CTX_DEP_MAX_F1 and context_str and len(ctx_dep_items) < n_target:
