"""
experiments/rlhf_attenuation_mistral_v1/rlhf_attenuation_mistral_v1.py

RLHF ATTENUATION — Mistral-7B (SWA)
======================================

Single-family script. No multi-family memory leak risk.

Hypothesis: Δ = instruct_auroc − base_auroc.
SWA (Sliding Window Attention) is the third architecture family after GQA and MQA.
Unknown whether Δ ≈ −0.036 (GQA pattern) or something different.

Models:
  Base:    mistralai/Mistral-7B-v0.1
  Instruct: mistralai/Mistral-7B-Instruct-v0.3

Architecture note: SWA uses alternating full/sliding-window attention layers.
The epistemic geometry may be distributed differently across layers vs GQA.
We use L(n-2) as the probe layer (layer 30 for 32-layer Mistral).

Prior results by architecture:
  GQA: Δ = -0.036 (Qwen2.5-1.5B, Llama-3.2-3B) — consistent, small attenuation
  MQA: Δ = -0.183 (Gemma-2-2B) — near-zero baseline, direction rotation (cosim=0.054)
  SWA: Δ = ? ← this experiment
...
BASE_ID = _KG_MISTRAL if os.path.exists(_KG_MISTRAL) else "mistralai/Mistral-7B-v0.1"
INST_ID = "mistralai/Mistral-7B-Instruct-v0.3"
ARCH    = "SWA"

OUT_DIR  = Path("/kaggle/working")
OUT_FILE = OUT_DIR / "rlhf_attenuation_mistral_v1_results.json"
...
def load_model(model_id: str):
    ...
    # device_map="auto" avoids materializing full fp16 before quantization (T4 OOM fix).
...
def calibrate(model, tok, data: List[Dict], layer_idx: int,
    ...
    for i, s in enumerate(shuffled):
        ...
        # Mistral base uses completion format; instruct uses chat template
        if "Instruct" in model_id or "instruct" in model_id:
            try:
...
def main():
    t0 = time.time()
    print(f"RLHF Attenuation — Mistral-7B (SWA)  |  {time.strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    print(f"Base: {BASE_ID}", flush=True)
    print(f"Inst: {INST_ID}", flush=True)
    print(f"Architecture: SWA (Sliding Window Attention)", flush=True)
    print(f"Prior: GQA Δ=-0.036 | MQA Δ=-0.183 (cosim=0.054 near-orthogonal)", flush=True)
    ...
    try:
        ...
        base_auroc = auroc
        base_dir   = d
        results.update({"base_auroc": round(auroc, 4), "base_mu_p": round(mu_p, 3),
    ...
    try:
        ...
        inst_auroc = auroc
        inst_dir   = d
        results.update({"inst_auroc": round(auroc, 4), "inst_mu_p": round(mu_p, 3),
    ...
    if base_auroc is not None and inst_auroc is not None:
        ...
        if abs(delta - (-0.036)) < 0.020:
            verdict = "GQA_PATTERN_CONFIRMED"  # SWA behaves like GQA
        elif delta < -0.060:
