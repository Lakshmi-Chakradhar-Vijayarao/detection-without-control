#!/usr/bin/env python3
"""
phi_bilateral_v1.py — EXP_PHI_BILATERAL_V1
Fifth-architecture replication of bilateral oracle L1 + L2 false certainty
experiment on Phi-3.5-Mini-Instruct (Microsoft).

SCIENTIFIC QUESTION:
  Does bilateral oracle Fisher+PCA64 replicate on Phi-3.5-Mini-Instruct?
...
ARCHITECTURE CONTEXT:
  This is the 5th architecture tested for architecture generalization:
    1. Qwen2.5-3B-Instruct     (COMMITTED_EARLY confirmed)
    2. Llama-3.2-3B-Instruct   (COMMITTED_EARLY confirmed)
    3. Gemma-2-2B-IT           (bilateral oracle L1 AUROC=0.7528)
    4. Mistral-7B-Instruct     (bilateral oracle replicated)
    5. Phi-3.5-Mini-Instruct   ← THIS EXPERIMENT
...
DESIGN:
  Phase 1 — Bilateral Oracle L1 (N=200/class, pool=15000, shuffled control):
    ...
    Fisher+PCA64 at penultimate layer (layer 30 of 32) step-1.
  ...
  Phase 2 — L2 False Certainty (N=100/class, entropy-matched):
    From pool, collect CC (correct, low entropy) and CW (wrong, low entropy)
    items with entropy in calibrated window [THETA_LO, THETA_HI].
    Fisher+PCA64 vs entropy baseline. Gap = Fisher - Entropy.
    Entropy fix: cast logits to float32, apply nan_to_num before softmax,
    filter NaN from calibration sample.
...
TERMINATION RULES:
  ...
  T2: L2 gap < 0.05 → L2 does not generalize to Phi, note scope contraction

GPU: T4 (~8 hrs)
Model: microsoft/Phi-3.5-mini-instruct (3.8B, float16, no quantization needed)
Hidden dim: 3072, 32 layers, probe at layer 30 (penultimate).
trust_remote_code=True required.
"""

from __future__ import annotations
import subprocess as _sp
# DynamicCache.from_legacy_cache removed in transformers>=4.47; pin to last compatible
_sp.run(["pip", "install", "-q", "transformers==4.46.3"], check=True)
...
LAYER_IDX        = None          # set in main() as n_layers - 2 (penultimate = 30)
...
CTX_MIN_F1_WC    = 0.50
CW_MAX_F1        = 0.05
ENT_HALF         = 0.30          # half-width of entropy window; calibrated below
CALIB_SAMPLE     = 500

SAVE_PATH        = "/kaggle/working/phi_bilateral_v1_results.json"
INTERMEDIATE     = "/kaggle/working/phi_bilateral_v1_intermediate.json"
...
def load_model():
    ...
    # Phi-3.5-Mini: 3.8B fits on T4 in float16 — no device_map="auto", no quantization
...
# ── Layer resolver (Phi-3.5-Mini uses model.model.layers standard HF layout) ────
_LAYER_PATHS = [
    "model.layers",
    "model.model.layers",
...
# ── Hidden-state + entropy extraction ───────────────────────────────────────────
def get_step1_hs(model, tokenizer, prompt: str, layer_idx: int):
    """Extract hidden state at layer_idx, last token, step-1 (prompt only)."""
...
def get_step1_entropy(model, tokenizer, prompt: str) -> float:
    """
    Compute step-1 token entropy at the last prompt position.

    CRITICAL FIX (same as gemma_l2_v1):
      - Cast logits to float32 to avoid NaN from float16 overflow.
      - Apply nan_to_num before softmax to clamp any remaining inf/nan.
      Returns 0.0 for non-finite result rather than propagating NaN.
    """
    ids = tokenizer(prompt, return_tensors="pt").input_ids.to(DEVICE)
    with torch.no_grad():
        out = model(ids)
...
def get_hs_and_entropy(model, tokenizer, prompt: str, layer_idx: int):
    """
    Combined extraction: hidden state at layer_idx + step-1 entropy.
    Single forward pass with a hook. Uses the same float32 entropy fix.
    """
    ids = tokenizer(prompt, return_tensors="pt").input_ids.to(DEVICE)
    hs_out = [None]
...
def run_phase1(model, tokenizer, pool, layer_idx):
    ...
    param_idx = np.where(y == 1)[0]
    ctx_idx   = np.where(y == 0)[0]
...
def calibrate_theta(model, tokenizer, pool, layer_idx):
    """
    Calibrate entropy threshold on pool sample.

    Uses the same float32 / nan_to_num fix as gemma_l2_v1:
      - Filters non-finite and zero entropy values from calibration sample.
      - Hard errors if all values are NaN.
    """
    print("  Calibrating entropy threshold …", flush=True)
    sample_ents = []
    for item in pool[:CALIB_SAMPLE]:
        pnc = prompt_nocontext(tokenizer, item["question"])
        _, ent = get_hs_and_entropy(model, tokenizer, pnc, layer_idx)
        if np.isfinite(ent) and ent > 0:
            sample_ents.append(ent)
    if not sample_ents:
        raise RuntimeError("All entropy values NaN/zero during calibration. "
                           "Check model logits and float32 cast.")
...
def run_phase2(model, tokenizer, pool, layer_idx, bo_pca, bo_lda):
    ...
    theta_conf, ent_lo, ent_hi = calibrate_theta(model, tokenizer, pool, layer_idx)
    ...
    if len(cc_hs) < 10 or len(cw_hs) < 10:
        ...
        return {
            "l2_skip":  True,
            "l2_reason": "Insufficient items in entropy window",
            "theta_conf": theta_conf,
            "ent_window": [ent_lo, ent_hi],
        }
    ...
    # Entropy AUROC on L2 task (lower entropy → more likely CC)
    from sklearn.metrics import roc_auc_score
    ...
    # BO Transfer AUROC: apply Phase 1 probe to CC/CW test items
    try:
        bo_transfer, _ = eval_probe(bo_pca, bo_lda, X_test, y_test)
    except Exception:
        bo_transfer = None

    shuf_status = "CLEAN" if fisher_shuf < 0.62 else ("WARN" if fisher_shuf < 0.70 else "FAIL")
    ...
    print(f"L2 Fisher={fisher_auroc:.4f}  CI=[{fisher_ci[0]:.3f},{fisher_ci[1]:.3f}]", flush=True)
    print(f"L2 Entropy={ent_auroc:.4f}  Gap={gap:.4f}", flush=True)
    print(f"L2 shuffled={fisher_shuf:.4f} ({shuf_status})", flush=True)
    if bo_transfer is not None:
        print(f"L2 BO_Transfer={bo_transfer:.4f}", flush=True)
    else:
        print("L2 BO_Transfer=N/A", flush=True)
    ...
    return {
        ...
        "theta_conf":        theta_conf,
        "ent_window":        [ent_lo, ent_hi],
        "cc_ent_mean":       cc_ent_mean,
        "cw_ent_mean":       cw_ent_mean,
        "fisher_auroc":      fisher_auroc,
        "fisher_ci_95":      list(fisher_ci),
...
def main():
    ...
    # Set layer index to penultimate (30 for 32-layer Phi-3.5-Mini)
    global LAYER_IDX
    LAYER_IDX = n_layers - 2
    print(f"Probe layer: {LAYER_IDX} (penultimate of {n_layers})", flush=True)

    # Phase 1: L1 bilateral oracle
    l1_results, bo_pca, bo_lda, X_l1, y_l1 = run_phase1(model, tokenizer, pool, LAYER_IDX)

    # Intermediate save after Phase 1
    try:
        with open(INTERMEDIATE, "w") as f:
            json.dump({"phase1_complete": True, "l1": l1_results}, f, indent=2)
    except Exception:
        pass
    ...
    # Assemble results
    results = {
        "experiment":         "EXP_PHI_BILATERAL_V1",
        "model":              MODEL_ID,
        "layer_idx":          LAYER_IDX,
        "n_layers":           n_layers,
        "pca_dim":            PCA_DIM,
        ...
        "architecture_note":  (
            "Phi-3.5-Mini-Instruct (Microsoft): 3.8B params, 32 layers, "
            "hidden=3072, RoPE, SwiGLU, GQA. float16, no quantization. "
            "5th architecture in generalization sequence "
            "(Qwen, Llama, Gemma, Mistral, Phi)."
        ),
        "entropy_fix":        "float32 logit cast + nan_to_num before softmax",
    }
    ...
    if l1_auroc < 0.65:
        ...
        results["termination"] = "T2_L2: L2 gap < 0.05; L2 does not generalize to Phi"
    else:
        results["termination"] = "NO_TERMINATION"

    print(f"\n{'='*50}", flush=True)
    print(f"EXP_PHI_BILATERAL_V1 RESULTS", flush=True)
    ...
    if not l2_results.get("l2_skip"):
        ...
        bo_t = l2_results.get("bo_transfer_auroc")
        if bo_t is not None:
            print(f"  BO_Transfer = {bo_t:.4f}", flush=True)
    else:
        print(f"  L2 SKIPPED: {l2_results.get('l2_reason','')}", flush=True)
