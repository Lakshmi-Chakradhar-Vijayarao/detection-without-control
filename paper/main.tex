...
% ── Abstract ──────────────────────────────────────────────────────────────────
\begin{abstract}
The hidden-state direction that best separates confident-correct from confident-wrong
answers is \textbf{not} the direction that causally controls which answer the model
generates. We establish this dissociation using Fisher+PCA64 linear probes
(AUROC\,=\,0.854; gap\,=\,0.240 over single-pass entropy baseline; gap\,=\,0.166 over
self-consistency with $N$\,=\,5 samples---the strongest behavioral proxy;
$N$\,=\,100/class, entropy-matched) and centroid-patching interventions that produce max
$\Delta$F1\,=\,$+$0.0004 across all layers and patch magnitudes (EXP-H)---indistinguishable
from zero. This finding is possible because when both classes have identically low
output entropy, no output-space signal---entropy, calibration, self-consistency---can
separate them by construction; hidden states carry information that output distributions
do not.

We introduce the \textbf{bilateral oracle protocol}, a two-pass behavioral labeling
design that isolates the confabulation-detection task from knowledge-source routing
by construction, without accessing hidden states. In the entropy-matched zone where
output statistics are blind, a Fisher+PCA64 linear probe at L26 step-1 achieves
AUROC\,=\,0.854 and cross-validated AUROC\,=\,0.7629\,$\pm$\,0.0120
($N$\,=\,500/class; C040~\textsc{confirmed}). CO labeling extends this detection
result to three confirmed architectures (Gemma\,=\,0.8368, Mistral\,=\,0.8580;
C036~\textsc{confirmed}; Llama plain CO INCONCLUSIVE---C047).

To test whether this detection geometry \emph{controls} answer selection, we patch
along the Fisher LDA class-mean axis at $\lambda$\,=\,0.1--2.0 across all probed
layers (EXP-H). Max $\Delta$F1\,=\,$+$0.0004---indistinguishable from zero. This
is consistent with \citet{cox2026decoding}, who demonstrated causal control via
a \emph{different} direction (the committed-answer direction). The two axes are
empirically dissociable: one reads whether an answer is correct; the other writes
which answer gets generated.

For scope context: output entropy dominates Fisher at L1 (knowledge-source routing;
AUROC 0.87--0.90 vs.\ 0.73--0.85), confirming Fisher is essential only at L2.
Reasoning models commit to answer direction after 17--25\% of thinking tokens
across three architectures, supporting early-exit routing at the commit point.
...
The purpose of this work is not to introduce another probing method.
It is to establish experimental methodology for studying internal computation
independently of any specific estimator.
...
Reasoning-distilled models commit to their answer direction after processing 17--25\%
...
\item \emph{The probe class is expressive enough to detect the signal.}
  ...
  AUROCs are lower bounds on the true $O$ under this assumption. The SAE/MLP
...
Mistral-7B-Instruct-v0.2 & Mistral & 200 & 0.778 & 0.552 & [0.69, 0.86] \\
...
\subsection{CO Labeling: Universal L2 Recovery (C036)}
...
\midrule
Qwen2.5-1.5B-Instruct & 100 & 0.885 & 0.653 & $+$0.232 & CO\_RECOVERS \\
Gemma-2-2B-IT & 200 & 0.837 & 0.631 & $+$0.206 & CO\_RECOVERS \\
Mistral-7B-Instruct-v0.3 & 200 & 0.858 & 0.647 & $+$0.211 & CO\_RECOVERS \\
Llama (CV) & 500 & 0.763 & --- & $+$0.163 & CV STABLE \\
\bottomrule
\end{tabular}
\end{table}
...
requires testing on a reasoning model from a different distillation lineage
(EXP\_TEACHER\_INDEPENDENCE\_V1, planned).
...
(observational). EXP-I converts this to a causal test: for each of $N$\,=\,200
questions, generation is run twice---once to completion and once truncated at the
detected commit point. $\Delta$f1\,=\,f1\_full $-$ f1\_truncated.

\begin{table}[h!]
\centering
\caption{EXP-I early exit causal results (DeepSeek-R1-Distill-Qwen-1.5B, $N$\,=\,200).}
\begin{tabular}{lc}
\toprule
Metric & Value \\
\midrule
$n$ committed & 199/200 (99.5\%) \\
Mean $\Delta$f1 & $+$0.0059 $\pm$ 0.0474 SD \\
SE & 0.0034 \\
95\% CI & [$-$0.0007, $+$0.0125] \\
$p$ (two-tailed) & 0.08 \\
$p$ (one-tailed, pre-registered directional) & 0.04 \\
Helped ($\Delta > 0.05$) & 7.54\% \\
Hurt ($\Delta < -0.05$) & 4.02\% \\
Mean commit\% & \textbf{87.4\%} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Verdict: MINIMAL\_QUALITY\_LOSS (C028).} Truncating at commit saves 87\% of
thinking tokens at $+$0.006 F1 cost (borderline significant; running mean stable at
$+$0.004 to $+$0.006 from item 50 through 200). The 1.9:1 helped/hurt ratio confirms
elaboration is directionally beneficial but not essential. Together with EXP-H (centroid patching shows max $\Delta$F1\,=\,$+$0.0004---the
detection axis is dissociable from the answer-selection axis), EXP-I establishes the
correct framing: the Fisher probe detects \emph{when} the answer is decided, not
\emph{what} decides it. The commit point is a timing readout, not a causal control point.
...
EXP-H (centroid-direction patching at $\lambda$\,=\,0.1--2.0 across all probed layers)
...
$\lambda$\,=\,0.1--2.0 (this work), vs.\ $>$50\% flip rate for the committed-answer
...
\item[Observation 3 (Commitment Precedes Verbalization).]
  ...
  processing only 17--25\% of their think-block tokens (commit\%\,$\geq$\,75\%
...
EXP-I & Early exit causal? & MINIMAL: $+$0.006, 87.4\% savings & C028 \textsc{supported} \\
...
before it is verbalized. The causal experiment (EXP-I) establishes that truncating at the
commit point costs $+$0.006 F1 ($p$\,=\,0.08)---nearly neutral quality loss for 87.4\%
...
C025~\textsc{confirmed}: ICC\,=\,0.913/0.933 Qwen/Llama); truncating at the commit
point saves 87.4\% of compute at $+$0.006~F1 cost ($p$\,=\,0.08).
...
\textbf{Seven claims are confirmed} (C001--C004, C025, C036, C040), twenty-six are
supported, eight are exploratory, and four are falsified---all maintained in an open
claims registry (45 total; Appendix~\ref{app:claims}). The falsification record is not
...
\bibitem[Cheang et al.(2024)]{dollmsknow2024}
Cheang, C. S., Chan, H. P., Zhang, W., \& Deng, Y. (2024).
...
C024 & Detection axis (Fisher LDA class-mean) dissociable from answer-selection axis: max $\Delta$F1\,=\,$+$0.0004 across $\lambda$\,=\,0.1--2.0 under centroid patching (EXP-H) & SUPPORTED \\
...
\noindent\small\textit{Remaining 29 claims (C005--C007, C009--C011, C013, C015--C016, C018--C021,
C023, C026--C035, C037--C039, C041, C044) and all kill criteria: see
...
EXP\_I\_EARLY\_EXIT & Early exit causal: $\Delta$f1\,=\,$+$0.006, 87.4\% savings. C028. & COMPLETE \\
...
tokens are nearly neutral elaboration: $\mathbb{E}[\Delta\text{F1}]$\,=\,$+$0.006
(C028).
...
\begin{itemize}
  ...
  \item \textbf{Regularity 4:} $O(M, L_{\text{peak}}, 1, T_{\text{MATH}}) \geq 0.85$ for $M \in \mathcal{M}_{\text{reasoning}}$. \emph{Supported, single architecture.}
\end{itemize}
