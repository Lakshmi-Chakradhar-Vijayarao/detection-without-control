...
\item \textbf{Bilateral oracle protocol.}
  A two-pass behavioral labeling design assigns \textsc{param}/\textsc{ctx\_dep}
  labels without accessing hidden states, creating an entropy-matched zone where
  all output-space signals are blind by construction. This is the enabling
  methodological contribution: it makes the L2 confabulation-detection task tractable.

\item \textbf{Confabulation geometry (L2).}
  In the entropy-matched zone, Fisher+PCA64 at L26 step-1 achieves AUROC\,=\,0.854
  (gap\,=\,0.240 over entropy; CV\,=\,0.7629\,$\pm$\,0.0120, $N$\,=\,500/class;
  C040~\textsc{confirmed}). CO labeling extends detection to three confirmed
  architectures (Gemma\,=\,0.8368, Mistral\,=\,0.8580; C036~\textsc{confirmed};
  Llama CO INCONCLUSIVE---C047).

\item \textbf{Causal dissociation (EXP-H).}
  Centroid patching along the Fisher LDA class-mean axis at $\lambda$\,=\,0--4.0
  across all probed layers yields max $\Delta$F1\,=\,$+$0.0004---indistinguishable
  from zero. The direction that reads correctness does not write answers,
  consistent with \citet{cox2026decoding}.

\item \textbf{Commit-point causal validation (L3, EXP-I).}
  Reasoning models commit to answer direction within 25\% of think tokens across
  three architectures and two training lineages. Truncating at the commit point
  costs $+$0.012~F1 at 90\% token savings---a causal result, not observational
  ($N$\,=\,500, $t$\,=\,6.0, $p$\,$<$\,0.0001; C028~\textsc{confirmed}).
\end{enumerate}

\noindent\textbf{Scope boundary.}
At L1 (knowledge-source routing), output entropy achieves 0.87--0.90 AUROC and
Fisher adds little---the task is a confident/uncertain distinction and Fisher is
redundant with entropy. Fisher is essential only at L2, where the entropy-matched
zone forces hidden-state measurement.
...
probes achieve 0.79~AUROC from step~1 for detecting CoT reasoning errors (peak 0.95 across all steps), yet
...
\textbf{Eight claims are confirmed} (C001--C004, C025, C028, C036, C040), thirty-two are
supported, five are exploratory, four are falsified, and one is inconclusive
(C047)---all maintained in an open claims registry (50 total; Appendix~\ref{app:claims}). The falsification record is not
...
\centering
\small
\caption{Core claims registry (17 claims). Full registry: \texttt{science/CLAIMS.yaml}. PARAM/CTX\_DEP abbreviated P/C.}
...
\noindent\small\textit{Remaining 33 claims (C005--C007, C009--C011, C013, C015--C016, C018--C021,
C023, C026--C027, C029--C035, C037--C039, C041, C044, C046--C050) and all kill criteria: see
