...
$N$\,=\,100/class, entropy-matched; both gaps from held-out evaluation, Fisher\,=\,0.779) and
centroid-patching interventions that produce max
$\Delta$F1\,=\,$+$0.0004 across all layers and patch magnitudes $\lambda$\,=\,0--4.0 (EXP-H)---indistinguishable
...
along the Fisher LDA class-mean axis at $\lambda$\,=\,0--4.0 across all probed
...
Reasoning models commit to answer direction within 25\% of thinking tokens
...
reported as such). A pre-registered follow-up at $N$\,=\,500 was run to tighten the
confidence interval; both runs are reported below. No result is selectively
suppressed. If the $N$\,=\,500 run does not clear $p$\,<\,0.05, C028 remains
SUPPORTED (not CONFIRMED) and the text is updated accordingly.
...
\midrule
$n$ committed & 199/200 (99.5\%) & \textit{[pending]} \\
Mean $\Delta$f1 & $+$0.0059 $\pm$ 0.0474 SD & \textit{[pending]} \\
SE & 0.0034 & \textit{[pending]} \\
95\% CI & [$-$0.0007, $+$0.0125] & \textit{[pending]} \\
$p$ (two-tailed) & 0.08 & \textit{[pending]} \\
$p$ (Wilcoxon, two-tailed) & --- & \textit{[pending]} \\
Helped ($\Delta > 0.05$) & 7.54\% & \textit{[pending]} \\
Hurt ($\Delta < -0.05$) & 4.02\% & \textit{[pending]} \\
Mean commit\% & \textbf{87.4\%} & \textit{[pending]} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Verdict: MINIMAL\_QUALITY\_LOSS (C028, SUPPORTED).} Run 1 shows truncating
at commit saves 87\% of thinking tokens at $+$0.006 F1 cost. The effect is
directionally consistent (1.9:1 helped/hurt ratio; running mean stable at
$+$0.004 to $+$0.006 from item 50 through 200) but borderline by two-tailed
threshold. Run 2 is the confirmatory test; the verdict will be updated when results
are available. Together with EXP-H (centroid patching shows max $\Delta$F1\,=\,$+$0.0004---the
...
\textbf{Seven claims are confirmed} (C001--C004, C025, C036, C040), twenty-nine are
